...
public:
	...
	DMAVideoBuffer(int lines, int lineSize, int clones = 1, bool ring = true, bool psram = true, int bufferCount = 1) {
		...
		for (int i = 0; i < bufferCount; i++) {
			unsigned char* pBuf;
			if (psram)
				pBuf = (unsigned char*)heap_caps_aligned_calloc(ALIGNMENT_PSRAM, 1, lineSize * lines, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
			else
				pBuf = (unsigned char*)heap_caps_aligned_calloc(ALIGNMENT_SRAM, 1, lineSize * lines, MALLOC_CAP_INTERNAL | MALLOC_CAP_DMA | MALLOC_CAP_8BIT);
			for (int y = 0; y < lines; y++) {
				buffer[i][y] = &pBuf[lineSize * y];
				if (!buffer[i][y]) {
					//TODO f this
